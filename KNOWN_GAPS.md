...
| ID | Gap | Why / Status | Resolution |
|----|-----|--------------|------------|
| KG-G-01 | Hub / shroud contours are a fixed canonical family | `cascade.geometry` ships tangency-correct quarter-ellipse hub and shroud contours (axial at the inducer / radial at the exit for a CC; mirrored for an RIT — Aungier 2000 §5.3 canonical shape, replacing the pre-KG-G-12 interpolating B-splines that rippled). The geometry design calls for user-manipulable contours and preset profiles. | v1.1: expose `hub_control_points` and `shroud_control_points` fields on `CentrifugalCompressorGeometry` / `RadialTurbineGeometry` so the meanline solver can hand them through and the UI can drag them. |
...
| KG-G-04 | No *deliberate* lean / sweep parameter on the blade | The camber is integrated per spanwise streamline (KG-G-12), so the blade carries the natural twist and rake of a ruled radial-machine blade — but there is no user parameter to add deliberate lean for stress / vibration tuning. | v1.1: lean parameter on the geometry dataclass. |
...
| KG-G-12 | ~~Blade camber copied the hub wrap to the shroud; contours rippled; splitters doubled the blade count~~ **CLOSED (2026-06-10)** | Four compounding defects made generated wheels look (and be) un-manufacturable: (1) the camber wrap was integrated only along the hub streamline with a fixed 60° LE angle and copied to the shroud — ~185° of hub wrap and an implied near-tangential (~83°) shroud blade, a corkscrew no flank mill can cut; (2) the interpolating-B-spline contours with bumped control points (1.05×/1.10× r at the inlet) produced non-monotonic radii and curvature ripples; (3) `with_splitter=True` rendered Z full + Z splitter blades, doubling the metal the meanline was scored with; (4) the cosmetic shroud cup defaulted ON, encasing the wheel in the viewer and leaving open surfaces in STL exports. Fixed: per-streamline camber integration with `tan β₁ ∝ r` LE twist (`passage_camber_grid`, validated against RadialDesigner's built-hardware generator), tangency-correct quarter-ellipse contours, the `blade_count` budget split Z/2 + Z/2 when splitters are on, and `with_shroud` defaulted off. TurboGrid/NDF and fluid-volume exports share the same curves and angles. Pinned by `tests/geometry/test_blade_camber.py`. | **Closed** |
